...
Any non-framework prop also accepts a function of the event input, and `children` accepts a function that returns `RemixNode`. With several sources the input `detail` is a tuple index-aligned with `eventSource`; an optional `initial` prop supplies the input rendered before an occurrence first matches. Server rendering resolves the initial input only; subscriptions are client-side.
