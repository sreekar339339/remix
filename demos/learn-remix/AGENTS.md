...
- Host elements are event-aware via the `eventSource` prop (`packages/ui/src/runtime/event-source.ts`): `<select eventSource={[events.a, events.b]} value={(selectedId) => ...}>{([people, prefix]) => ...}</select>`. Any prop and `children` may be a function of the event input, with the matched event as an optional second argument; the element re-renders through the vdom on matched events. `app/assets/utils/customEvents/` sources implement the protocol, and `evented.<tag>` is a type-only intrinsic alias (`evented.button` is the string `'button'` at runtime) that preserves typed callback inference over `eventSource`, inferring the event map from the descriptor passed as `eventSource`; the descriptor itself is the explicit wildcard source (`eventSource={events}`), which always reads the whole composite. Canonical components use remembered descriptors (`customEvents(seeds, folds)`): the descriptor is its own `EventTarget`, writes are `events.dispatchEvent({ seed: value } | 'occurrence')` (or a native `event`/`events.create(...)` on any target), fold events mutate an Immer draft of the composite with diff-based patch routing, and reads happen only through views. `customEvents<Definition>()` builds occurrence-only descriptors; `.asHost()` is the element-host mixin and `.asHost(target)` bridges a domain `EventTarget`.
- Keyed collections render through the `evented.list` intrinsic (`packages/ui/src/runtime/event-route.ts`): one state source, a per-item `(item, key) =>` template. Remembered descriptors attach structural routes (`EVENT_ROUTES`) to dispatched events in `app/assets/utils/customEvents/runtime.ts` (`notifyEntries`), so adds, removals, and Map value replaces apply fine-grained while whole-key changes reconcile through the keyed diff.
