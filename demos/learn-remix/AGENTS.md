...
- Host elements are event-aware via the `eventSource` prop (`packages/ui/src/runtime/event-source.ts`): `<select eventSource={[events.a, events.b]} value={(selectedId) => ...}>{([people, prefix]) => ...}</select>`. Any prop and `children` may be a function of the event input, with the matched event as an optional second argument; the element re-renders through the vdom on matched events. `app/assets/utils/customEvents/` sources implement the protocol, and `evented.<tag>` is a type-only intrinsic alias (`evented.button` is the string `'button'` at runtime) that preserves typed callback inference over `eventSource`, inferring the event map from the descriptor passed as `eventSource`; the descriptor itself is the explicit wildcard source (`eventSource={events}`), which always reads the whole composite on a retained descriptor or store. Canonical components use retained descriptors (`customEvents(seeds, folds)`), where the root event's detail is the composite of all held seeds, writes are `events.dispatch({ seed: value } | 'occurrence')`, effect events fold `(held, detail) => Partial<Held>` with diff-based patch routing, and reads happen only through views; `.store()` is the state-shaped addon for shared context (appContext), keeping `state.value`/`state.update`.
- Keyed collections render through the `evented.list` intrinsic (`packages/ui/src/runtime/event-route.ts`): one state source, a per-item `(item, key) =>` template. Retained descriptors attach structural routes (`EVENT_ROUTES`) to dispatched events in `app/assets/utils/customEvents/runtime.ts` (`notifyEntries`), so adds, removals, and Map value replaces apply fine-grained while whole-key changes reconcile through the keyed diff.
