...
- Host elements are event-aware via the `on` prop (`packages/ui/src/runtime/event-source.ts`): `<select on={[events.a, events.b]} value={(selectedId) => ...}>{([people, prefix]) => ...}</select>`. Any prop and `children` may be a function of the event input, with the matched event as an optional second argument; the element re-renders through the vdom on matched events. `app/assets/utils/customEvents/` sources implement the protocol, and `evented.<tag>` is a type-only intrinsic alias (`evented.button` is the string `'button'` at runtime) that preserves typed callback inference over `on`, inferring the event map from the descriptor passed as `on`; the descriptor itself is the explicit wildcard source (`on={events}`), which always reads the whole composite. Canonical components use remembered descriptors (`customEvents(details, folds)`): the descriptor carries a native `EventTarget` channel, writes are `events.dispatchEvent({ detailName: value } | 'occurrence')` (or a function of the composite returning the input, or a native `event`/`events.create({ name: detail })` on any target), fold events mutate an Immer draft of the composite with diff-based patch routing, and reads happen only through views. `customEvents<Definition>()` and `customEvents({ occurrence: recipe, ... })` build occurrence-only descriptors; `.asHost()` is the element-host mixin and `.asHost(target)` bridges a domain `EventTarget`.
